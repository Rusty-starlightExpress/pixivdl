...
illust_recovery = json.load(f)
f.close()

illust_file_ids = illust_recovery["error-data"]

try:
    for i in illust_file_ids:
        illust_ids.append(i["illust_id"])
except:
    print("\nたぶん終わり\n")
...
if loop > 1:
    for x in range(loop):
        sleep(10)
        illust_detail = aapi.illust_detail(illust_ids[x])
        user_name = illust_detail.illust.user.name
        user_id = illust_detail.illust.user.id
        total_works = illust_detail.illust.page_count
            ...
            try:
                ...
                if illust.type == "illust":
                    if illust.page_count == 1:
                        ...
                        frames += glob.glob(f'{saving_direcory_path}/{illust.id}_p*.jpeg')
                        frames += glob.glob(f'{saving_direcory_path}/{illust.id}_p*.png')
                        print("frames count : {}".format(len(frames)))
                        for frame in frames:
                            if debug:
                                print("process 2")
                                print("frame        : " + frame)
